...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; ftrace_write(s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1; ftrace_write(s->pc, s->dnpc));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , IS, R(rd) = (uint32_t)src1 >> imm);
